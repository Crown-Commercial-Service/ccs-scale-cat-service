...
import uk.gov.crowncommercial.dts.scale.cat.model.generated.CaTMessage;
import uk.gov.crowncommercial.dts.scale.cat.model.generated.CaTMessageNonOCDS;
import uk.gov.crowncommercial.dts.scale.cat.model.generated.CaTMessageOCDS;
...
import uk.gov.crowncommercial.dts.scale.cat.model.generated.MessageSummary;
import uk.gov.crowncommercial.dts.scale.cat.model.generated.MessageTotals;
...
public class MessageService {
   ...
   */
  public String sendOrRespondMessage(final String profile, final Integer projectId, final String eventId,
   ...
   */
  private String validateSuppliers(final ProcurementEvent procurementEvent, final MessageNonOCDS nonOCDS) {
    ...
    if (isEmpty(supplierOrgMappings)) {
      var errorDesc = String.format("No supplier organisation mappings found in Tenders DB %s", supplierOrgIds);
      log.error(errorDesc);
    ...
    if (!isEmpty(unMatchedSuppliers)) {
      var errorDesc = String.format("Supplied organisation mappings not matched with actual suppliers '%s'",
          unMatchedSuppliers);
      log.error(errorDesc);
      throw new JaggaerRPAException(errorDesc);
    }
    ...
    var matchedSuppliers = suppliers.stream()
        .filter(
            supplier -> supplierExternalIds.stream().anyMatch(orgId -> orgId.equals(supplier.getCompanyData().getId())))
        .collect(Collectors.toList());
  ...
  private String callRPAMessageAPI(final RPAProcessInput processInput) {
    ...
    request.setProcessInput(objectMapper.writeValueAsString(processInput))
        .setProcessName(RPAProcessNameEnum.BUYER_MESSAGING.getValue()).setProfileName(rpaAPIConfig.getProfileName())
  ...
  public MessageSummary getMessagesSummary(final MessageRequestInfo messageRequestInfo) {
    ...
    Predicate<uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message> directionPredicate = message -> (MessageDirection.ALL
        .equals(messageRequestInfo.getMessageDirection())
    ...
    Predicate<Receiver> receiverPredicate = receiver -> MessageRead.ALL.equals(messageRequestInfo.getMessageRead())
        || ((MessageRead.READ.equals(messageRequestInfo.getMessageRead()) && receiver.getId().equals(jaggaerUserId)));

    var messagesResponse = jaggaerService.getMessages(event.getExternalReferenceId(), messageRequestInfo.getPage());
    var allMessages = messagesResponse.getMessageList().getMessage();

    /**
     * Make first request to jagger if total records are more than > 100 (Jaggaer
     * returns max 100 order by date desc) and messageSort is TITLE/AUTHOR then make
     * sub-sequent call to get total records, then messageSort it send only
     * requested no of records
     */
    ...
    return new MessageSummary()
        ...
        .links(getLinks(messages, messageRequestInfo.getPageSize())).messages(getCatMessages(messages,
            messageRequestInfo.getMessageRead(), jaggaerUserId, messageRequestInfo.getPageSize()));
  }
  ...
  private Links1 getLinks(final List<uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message> messages,
    ...
    return new Links1().first(URI.create(LINK_URI + (message.getMessageId())))
        .self(URI.create(LINK_URI + (message.getMessageId()))).prev(URI.create(LINK_URI + (message.getMessageId())))
   ...
   */
  public uk.gov.crowncommercial.dts.scale.cat.model.generated.Message getMessageSummary(final Integer procId,
      final String eventId, final String messageId, final String principal) {
    ...
    return new uk.gov.crowncommercial.dts.scale.cat.model.generated.Message().OCDS(getMessageOCDS(reponse))
        .nonOCDS(getMessageNonOCDS(reponse));
  }

  private List<CaTMessage> getCatMessages(
      final List<uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message> messages, final MessageRead messageRead,
      final String jaggaerUserId, final Integer pageSize) {
  ...
  private CaTMessage convertMessageToCatMessage(
      final uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message message, final MessageRead messageRead,
  ...
  private CaTMessageOCDS getCaTMessageOCDS(uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message message) {
    return new CaTMessageOCDS().date(message.getSendDate()).id(message.getMessageId()).title(message.getSubject())
  ...
  private MessageOCDS getMessageOCDS(uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message message) {
    return new MessageOCDS().date(message.getSendDate()).id(message.getMessageId()).title(message.getSubject())
        .author(message.getSender().getName());
  }
  ...
  private MessageNonOCDS getMessageNonOCDS(uk.gov.crowncommercial.dts.scale.cat.model.jaggaer.Message message) {
    ...
    return new MessageNonOCDS().direction(MessageNonOCDS.DirectionEnum.fromValue(message.getDirection()))
...
//            .map(object -> new MessageNonOCDSAllOfAttachments()).collect(Collectors.toList()))
        .readList(message.getReadingList().getReading().stream()
            .map(reading -> new ContactPoint1().name(reading.getReaderName())).collect(Collectors.toList()))
