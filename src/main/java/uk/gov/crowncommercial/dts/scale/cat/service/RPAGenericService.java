...
public class RPAGenericService {
   ...
   */
  public Pair<List<Supplier>, Set<OrganisationMapping>> getValidSuppliers(final ProcurementEvent procurementEvent,
      final List<String> orgIds) {
    // ignoring string content from organisation Ids
    var supplierOrgIds = orgIds.stream().map(ls -> ls.replace("GB-COH-", "")).collect(Collectors.toSet());
   ...
   */
  @SneakyThrows
  public String callRPAMessageAPI(final RPAProcessInput processInput, final RPAProcessNameEnum processName) {
    var request = new RPAGenericData();
    request.setProcessInput(objectMapper.writeValueAsString(processInput)).setProcessName(processName.getValue())
        .setProfileName(rpaAPIConfig.getProfileName()).setSource(rpaAPIConfig.getSource())
        .setSourceId(rpaAPIConfig.getSourceId()).setRequestTimeout(rpaAPIConfig.getRequestTimeout()).setSync(true);
